   ...
   <div>
      <div class="row">
         <div class="col m9 offset-m1">
            <h2>Post by tag     {{$route.params.tag}}</h2>
         ...
         <div class="col m2 hide-on-med-and-up">
            ...
            <ul class="collection with-header">
               <li class="collection-header">
                  <div class="z-depth-4" style="background:#EE6E73;margin:20px;height:50px;text-align:center;border:1px solid #EE6E73;border-radius:5px;display:flex;align-items:center;justify-content:center;font-size:20px;font-family: 'Roboto', sans-serif;color:white">Filtered for</div>
               </li>

               <li v-for="tag in tagComp" :key="tag" class="collection-item">{{tag}}</li>
...
<script>
   ...
   export default{
       data:function(){
           ...
           var r= this.$route.params.tag;
           postsRef.where('content.tag','==',r).get().then(doc=>{
               console.log(doc);
       doc.docs.forEach(d=>{
           var data=d.data();
